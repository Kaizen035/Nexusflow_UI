--[[ 
    NEXUSFLOW UI LIBRARY - REFINED EDITION (v19.1 Stable)
    Fixed & Optimized by Gemini
    
    Changelogs:
    - Fixed: SetProfileVisible causing errors (Changed Frame to CanvasGroup).
    - Added: Safe GUI Parenting (Works on Solara, Incognito, standard Studio).
    - Optimized: Dropdown layout and scrolling.
    ...
    NotifyFunc = function(Title, Text, Duration)
        ...
        task.delay(Duration or 3, function()
            if Frame and Frame.Parent then
                local OutTween = TweenService:Create(Frame, TweenInfo.new(NexusFlow.AnimSpeed), {Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1})
...
function NexusFlow:CreateWindow(Config)
    ...
    if TargetParent:FindFirstChild("NexusFlow_Main") then TargetParent.NexusFlow_Main:Destroy() end

    local ScreenGui = Create("ScreenGui", {Name = "NexusFlow_Main", Parent = TargetParent, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    
    -- Protect GUI if using an exploit that supports it
    ...
    -- Profile Section (FIXED: Using CanvasGroup for GroupTransparency)
    local ProfileFrame = Create("CanvasGroup", {
        Parent = Sidebar, BackgroundTransparency = 1, Position = UDim2.new(0, 0, 1, -50), 
        Size = UDim2.new(1, 0, 0, 50), GroupTransparency = 0, BorderSizePixel = 0
    })
    ...
    task.spawn(function()
        ...
        if success then ProfileImg.Image = content end
    end)
                ...
                UserInputService.InputBegan:Connect(function(input) 
                    if Binding then
                        if input.UserInputType == Enum.UserInputType.Keyboard or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
                           ...
                           local KeyName = (typeof(Key) == "EnumItem" and Key.Name) or Key.Name
                           BindBtn.Text = KeyName
                           BindBtn.TextColor3 = NexusFlow.CurrentTheme.SubText 
                           Binding = false 
                           Callback(Key) 
                        end
                    end 
            ...
            function TargetObj:CreateDropdown(Text, Options, Callback)
                ...
                local function RefreshList() 
                    ...
                    for _, opt in pairs(Options) do 
                        local OptBtn = Create("TextButton", {Parent = ListFrame, BackgroundColor3 = NexusFlow.CurrentTheme.Main, Size = UDim2.new(1, 0, 0, 26), Text = opt, Font = Enum.Font.Gotham, TextSize = 11, TextColor3 = NexusFlow.CurrentTheme.SubText}) 
                        RegisterThemeObj(OptBtn, "BackgroundColor3", "Main") 
                        RegisterThemeObj(OptBtn, "TextColor3", "SubText") 
                        Create("UICorner", {Parent = OptBtn, CornerRadius = UDim.new(0, 4)}) 
                        OptBtn.MouseButton1Click:Connect(function() 
                            Dropped = false 
                            Lbl.Text = Text .. " : " .. opt 
                            Callback(opt) 
                            TweenService:Create(DropCont, TweenInfo.new(0.3), {Size = UDim2.new(1, 0, 0, 32)}):Play() 
                ...
                Btn.MouseButton1Click:Connect(function() 
                    Dropped = not Dropped 
                    local TargetHeight = Dropped and math.min(130, listLayout.AbsoluteContentSize.Y + 40) or 32 
                    TweenService:Create(DropCont, TweenInfo.new(0.3), {Size = UDim2.new(1, 0, 0, TargetHeight)}):Play() 
                    TweenService:Create(Icon, TweenInfo.new(0.3), {Rotation = Dropped and 180 or 0}):Play() 
                end)
            end
